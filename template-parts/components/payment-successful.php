<section class="flex justify-center items-center w-[92%] px-[4%] mx-auto">
    <div class="rounded-5 shadow-little-box bg-white flex flex-col items-center my-14 max-w-normal mx-auto w-full py-8 px-[4%] ">
        <div class="max-w-[692px] flex flex-col items-center">
            <img class="max-w-[147px]" src="<?= ASSETS . '/images/happy-customer.png' ?>" alt="">
            <h1 class="md:text-4xl text-2xl text-[#256BA2] font-cassanet py-8">
                <?= __( 'CONGRATULATIONS', 'meridian' ) ?>

            </h1>
            <p class="font-black md:text-2xl text-xl">
                <?= __( 'Your payment was successful.', 'meridian' ) ?>
            </p>
            <p class="md:text-xl text-lg text-[#256BA2]">
                <?= __( 'We are pleased to welcome you to Meridian School of English.', 'meridian' ) ?>
            </p>
            <p class="text-base my-7">
                <?= __( 'You will receive a copy of the invoice in your email soon.', 'meridian' ) ?>
            </p>
            <p class="text-base ">
                <?= __( 'We will also send you an email about <span class="text-blue">IMPORTANT STEPS TO COMPLETE TO JOIN THE CLASS</span>, you and your child will need to complete the 4 pre-lesson tasks before the deadline we provide in the email, otherwise we will have to defer your child’s start date.', 'meridian' ) ?>
            </p>
            <p class="text-base my-7">
                <?= __( 'If you have any questions before the course starts you can contact us at <strong>support@meridianenglish.com</strong>', 'meridian' ) ?>
            </p>
            <p class="text-base ">
                <?= __( 'Many thanks!', 'meridian' ) ?>
            </p>
