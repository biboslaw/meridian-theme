...
<?php wp_body_open(); ?>
<?php
	if( $current_language === 'cs' && is_front_page() ){
		?>
			<div id="myModal" class="fixed top-0 left-0 right-0 z-[3247483649] w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-full max-h-full flex justify-center items-center shadow bg-black bg-opacity-50 text-white">
				<div class="modal-content relative">
					<img class="sm:block hidden" src="<?= ASSETS . '/images/cs-modal-content.png' ?>" alt="">
					<img class="sm:hidden block" src="<?= ASSETS . '/images/cs-modal-content-mobile.png' ?>" alt="">
					<span class="wow-close absolute right-[0.8rem] top-[0.1rem] text-2xl cursor-pointer">&times;</span>
					<a class="hover:bg-[#8A1B61] py-2 px-10 bg-orange sm:bottom-[10%] bottom-[3%] sm:left-[7%] left-[17%] absolute sm:text-[24px] text-[16px] font-bold visited:text-white text-white rounded-[25px]" href="https://www.meridianenglish.com/cs/booking-cz/?startDate=13/03/2023&days=3&onlyproduct=3661727000059164150&hidden=true&promoCode=TRCMA99&idAgent=3661727000077171015">
						<?php echo __( 'Rezervujte nyni', 'meridian' ); ?> 
					</a>
				</div>
			</div>
		<?php
	...
	<div class="w-full sticky top-0 z-40 shadow-little-box">
		...
		<div class="bg-[#FFF2DD] w-full px-[2%]">
			<div class="flex justify-between <?php echo strtolower($current_language) == "cs" ? ' sm:h-12 h-20' : 'h-12' ?> items-center max-w-wide mx-auto flex-wrap">
				...
				<?php
					if(strtolower($current_language) == "cs") {
						?>
							<span class="text-[#535353] sm:w-auto w-full sm:order-none order-1 flex justify-center">
								<a href="https://www.meridianenglish.com/cs/akce-3lekce-za-99kc/">
									<span class="font-bold">
										<?php echo __( '3 ZKUŠEBNÍ lekce jen za 99 Kč! ', 'meridian' ); ?>
									</span>
									<span class="underline">
										<?php echo __( 'REZERVUJTE VÝHODNĚ »', 'meridian' ); ?>
									</span>
								</a>
							</span>
						<?php
